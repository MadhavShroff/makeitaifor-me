...
const MarqueeText = ({ text }) => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (ref.current) {
        ref.current.style.transform = `translateX(${scrollY * 0.75 - 2000}px)`;
      }

      if (strokeOffsetRef.current) {
        strokeOffsetRef.current.style.strokeDashoffset =
          4389.86 - scrollY * 2.4 + 1000;
      }
    };

    strokeOffsetRef.current.style.strokeDashoffset =
      4389.86 - window.scrollY * 2.2;
