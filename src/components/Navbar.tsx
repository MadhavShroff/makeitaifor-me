...
const defaultLinks: LinkProps[] = [
  ...
  { href: '/documents', text: 'Documents Lookup' },
...
const Navbar: React.FC<NavbarProps> = (props) => {
  ...
  return (
    ...
    >
      <Link
        href="/"
        className="px-2 py-1 sm:py-2 sm:text-xl text-2xl font-bold text-black hover:underline decoration-orange-500 decoration-4"
      >
        ...
        >
          {links.map((link, index) => (
            <Link
              key={index}
              href={link.href}
              className="px-3 sm:px-4 py-1 sm:py-1 text-lg sm:text-xl border rounded-full hover:bg-orange-500 whitespace-nowrap font-bold"
              style={{
          ...
          {user != null ? (
            <Link
              key={11}
              href={'/profile'}
              className="flex items-center pr-1 pt-1 pb-1 pl-3 text-lg sm:text-xl border rounded-full hover:bg-orange-500 whitespace-nowrap font-bold"
              style={{
                borderColor: 'var(--background-color)',
                color: 'var(--background-color)',
              }}
