...
const NavRow = ({text, onEditClicked, onDeleteClicked, onChatClicked, isSelected}) => {
    return <li className="pt-2 group">
        <button className={`flex w-full text-left py-3 px-3 items-center gap-3 relative rounded-md cursor-pointer ${isSelected ? "bg-orange-500" : "group-hover:bg-[#341100]"}`} onClick={onChatClicked} data-te-ripple-init >
            ...
            <div className="absolute flex right-1 z-10 text-white group-hover:visible">
                <button className="p-1" onClick={onEditClicked}>
                    <svg className="h-4 w-4" stroke="currentColor" fill="none" strokeWidth="2" viewBox="0 0 24 24" strokeLinecap="round" strokeLinejoin="round" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg">
                        <path d="M12 20h9" />
                ...
                <button className="p-1" onClick={onDeleteClicked}>
                    <svg className="h-4 w-4" stroke="currentColor" fill="none" strokeWidth="2" viewBox="0 0 24 24" strokeLinecap="round" strokeLinejoin="round" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg">
...
export const ChatComponentNav = ({ toggleSideNav, showSideNav } : { toggleSideNav: () => void, showSideNav: boolean }) => {
    ...
    return <div id="chat-nav" className={"z-20 bg-black absolute rounded top-[2px] left-[2px] " + (showSideNav ? "border-2" : "")}>
